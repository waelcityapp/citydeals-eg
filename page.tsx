  ...
  const renderHome = () => (
    <div className="flex flex-col pb-32 max-w-[430px] mx-auto bg-black text-white" dir="rtl">
      ...
      <div className="px-4 mt-5">
        <button className="w-full flex items-center justify-between bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-right transition-all active:bg-white/10 group">
          ...
          <ChevronDown className="h-[18px] w-[18px] text-white/50 group-hover:text-white transition-colors" />
        </button>
      </div>
      ...
      <div className="px-4 mt-6">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="bg-[#111] border border-white/5 rounded-2xl overflow-hidden shadow-2xl relative group">
            ...
            <button className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full bg-black/60 backdrop-blur-sm flex items-center justify-center text-gray-400 hover:text-red-500 transition-colors">
              <Heart className="h-4 w-4" />
            </button>
            <div className="relative h-48 w-full bg-gradient-to-br from-[#1e1e1e] to-[#121212] overflow-hidden">
              <img src="https://picsum.photos" alt="Koshary" className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-500" />
      ...
      <nav className="fixed bottom-4 left-4 right-4 bg-[#111]/90 backdrop-blur-lg border border-white/5 py-2.5 rounded-[24px] flex justify-around items-center shadow-2xl z-50 max-w-[398px] mx-auto">
        <button onClick={() => setCurrentTab("home")} className={`flex flex-col items-center gap-1 transition ${currentTab === "home" ? "text-red-500" : "text-gray-500 hover:text-white"}`}>
          <HomeIcon className="h-5 w-5" />
          <span className="text-[10px] font-bold">Home</span>
        </button>
        <button onClick={() => setCurrentTab("explore")} className={`flex flex-col items-center gap-1 transition ${currentTab === "explore" ? "text-red-500" : "text-gray-500 hover:text-white"}`}>
          <Compass className="h-5 w-5" />
          <span className="text-[10px] font-bold">Explore</span>
        </button>
        <div className="-mt-8 relative">
          <button className="h-14 w-14 bg-red-600 rounded-full flex items-center justify-center text-white shadow-[0_8px_24px_rgba(220,38,38,0.4)] hover:bg-red-500 hover:scale-105 transition border-4 border-black">
            <Search className="h-5 w-5" />
          </button>
        </div>
        <button onClick={() => setCurrentTab("saved")} className={`flex flex-col items-center gap-1 transition ${currentTab === "saved" ? "text-red-500" : "text-gray-500 hover:text-white"}`}>
          <Heart className="h-5 w-5" />
          <span className="text-[10px] font-bold">Saved</span>
        </button>
        <button onClick={() => setCurrentTab("profile")} className={`flex flex-col items-center gap-1 transition ${currentTab === "profile" ? "text-red-500" : "text-gray-500 hover:text-white"}`}>
